                ...
                <div className='w-full'>
                        ...
                        <div className='flex gap-2 items-center w-full'>
                            <figure className='w-10 h-10 rounded-full overflow-hidden cursor-pointer'>
                                {loggedUser?.profile !== null
                                    ? (
                                        <img src={loggedUser?.profile} alt="profile" className='w-full h-full object-cover object-center' />
                                    )
                                    : (
                            ...
                            <figcaption className='flex items-center'>
                                <div className='flex flex-col'>
                                    <span className={`${darkmode ? 'text-darkmode-text' : 'text-gray-700'}`}>{loggedUser?.username}</span>
                                    <span className={`text-xs ${darkmode ? 'text-darkmode-text/80' : 'text-gray-500'}`}>
                                        {new Date(loggedUser?.createdAt).toLocaleString('en-GB', {
                                            day: '2-digit',
                                            month: 'short',
                                            year: 'numeric',
                                            hour: '2-digit',
                    ...
                    <div className='mt-5'>
                        <figure className='w-full h-full cursor-pointer'>
                            <img src={loggedUser?.postFile.url} alt="profile" className='w-full h-full object-cover object-center' />
                        </figure>
